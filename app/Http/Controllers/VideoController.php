...
//            $imgsall=array_merge($imgs,$imgs2);
            ...
            foreach ($imgsall as $img) {
                $src = $img->src;
                if ((strpos($src, ".hiphotos.baidu") == false && strpos($src, ".baidu.com/it/u") != false)||strpos($src, ".gtimg.cn/") != false||strpos($src, ".ykimg.com/") != false||strpos($src, ".mgtv.com/") != false) {
                    array_push($srcs, $src);
...
//                    echo "<br>";
    ...
    public function getVideoByUserId(Request $request)
    {
        if (isset($_REQUEST["userid"])) {
            $uid = $_REQUEST["userid"];
            $key = null;
...
//            $pdo=new \PDO("","","");
                ...
                if ($ps->rowCount() > 0) {
                    ...
                    return DataHandlerUtil::returnJsonOnly(array("data" => $data));
                }
            } catch (\PDOException $e) {
                array_push($infos, $e->errorInfo);
                return DataHandlerUtil::returnJsonOnly(array("info" => $infos));
            }
        } else {
            array_push($infos, "userId is missing");
            return DataHandlerUtil::returnJsonOnly(array("info" => $infos));
        }
    }
}